...
contract Sadaiv {
  ...
  mapping(uint256 => mapping(string => Build)) public repoBuilds;
  ...
  function verifySignature(
    ...
    address calculatedAddress = address(
      uint160(uint256(keccak256(abi.encodePacked(signer)))) &
  ...
  function registerUser(
    ...
    require(
      verifySignature(message, signature, recoveredAddress),
      "Address not authorized!"
    );
    require(
      SCWAddressToSadaivId[recoveredAddress] != 0,
      "Address already registered with Sadaiv"
    );
    sadaiv_id.increment();
